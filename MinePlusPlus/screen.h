...
class Screen {
  ...
  public:
    void renderBitmap(const byte bitmap[], uint8_t rows, uint8_t columns, byte xPixel, byte yPixel) const;
    void renderBlock (xcoord_t x, ycoord_t y, int8_t xPixelOffset, int8_t yPixelOffset) const;
    void renderBlock (const CoordPair& coords, int8_t xPixelOffset = 0, int8_t yPixelOffset = 0) const;
    void renderWorld (bool reRenderAnimatedBlocks = false);
    void forceRenderWorld ();
    void renderWorldOverview (xcoord_t center) const;
