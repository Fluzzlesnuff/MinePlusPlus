...
class Mob { //A proper object. Used in arrays to control each mob.
  public:
    ...
    void damage (const id_t hearts); //decrease the number of hearts of the mob
    void kill (); //kill the mob, without inducing an item drop. Overridden per-mob to drop item(s).
    bool walk (const Direction dir); //Attempt to move the mob 1 block in the given direction. Returns true if the move is not obstructed by a solid block.
    void follow (const bool safety = true); //walk()s the mob one block towards the player, avoiding lava, damaging drops, and fire. follow(false) overrides the safety behavior.
    int drop (const id_t id, const id_t num = 1); //Summon items with the given number at the mob's FEET. Use multiple calls of drop() for multiple types of items
    int dropRand (const id_t id1, const id_t num1, const float chance1, const id_t num2 = 0, const float chance2 = 0, const id_t num3 = 0, const float chance3 = 0 );
...
class Hostile : public Mob {
    void attackMelee (const id_t hearts); //attempt to damage the player by touch
    void attackRanged (const id_t hearts, const id_t projectileId); //fires a projectile of the given type at the player
};
