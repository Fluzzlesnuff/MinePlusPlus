...
class Block { //Not used in the object sense. Instead, this is just a set of functions.
    ...
    bool scanToolTable (const id_t table[TT_ARRAY_SIZE], id_t& tool);
  ...
  public:
    ...
    id_t get (CoordPair coords); //Like above, but takes a CoordPair, most often from player.getCoords or mob.getCoords.
    bool set (xcoord_t x, ycoord_t y, id_t id); //Changes the block at a given coordinate to the given ID.
    bool set (CoordPair coords, id_t id); //Like above, but takes a CoordPair, most often from player.getCoords or mob.getCoords.
    ...
    bool place (CoordPair coords, id_t blockType);
    ...
    byte isTouchingWide(CoordPair coords, id_t id);
    uint16_t isNear(xcoord_t x, ycoord_t y, id_t id, byte distance, MeasurementType meausrementType); //Counts nearby blocks of the given type.
    ...
    */
    uint16_t isNear(CoordPair coords, id_t id, byte distance, MeasurementType measurementType);
